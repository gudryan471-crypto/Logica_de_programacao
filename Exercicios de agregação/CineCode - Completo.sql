	drop database if exists CineCode_gabarito;
    ...
    select * from clientes 
    where regiao = 'Sudeste' and sexo = 'feminino';
    
    select * from clientes 
    where estado = 'Sudeste';
    
    select * from filmes 
    where genero = 'Mistério e Suspense' or genero = 'Ficção e Fantasia';
    
    select * from filmes
    order by preco_aluguel desc limit 5;
    
    select * from clientes 
    order by data_conta limit 10;
    
    update filmes 
    set preco_aluguel = 3.50
    where id_filme = 71;
    ...
    where id_cliente = 109;
    
     select * from clientes
    where id_cliente = 109;
    
    update filmes 
    set preco_aluguel = 1.50
    ...
    where ano_lancamento > 2005 order by titulo; # faltou o sinal de ">"
    
    select nome_cliente, estado from clientes 
    where regiao = 'Sul'; # esta tentando selecionar os clientes do atributo região com o atributo estado
    
    select * from alugueis where nota is, not null
    
    select nome_ator, nacionalidade from atores;
    
    update filmes set genero = 'Fantasia'
    where titulo like 'Herry%';
